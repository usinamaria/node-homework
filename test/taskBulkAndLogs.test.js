...
describe("testing bulk update/delete by filter query parameters", () => {
  ...
  it("84. taskA and taskB are now marked completed; taskC (high priority) was not affected.", async () => {
    const refreshedA = await prisma.task.findUnique({
      where: { id: taskA.id },
    });
    const refreshedB = await prisma.task.findUnique({
      where: { id: taskB.id },
    });
    const refreshedC = await prisma.task.findUnique({
      where: { id: taskC.id },
    });
  ...
  it("88. taskC and user2's task still exist.", async () => {
    const remainingC = await prisma.task.findUnique({
      where: { id: taskC.id },
    });
    const remainingOther = await prisma.task.findUnique({
      where: { id: otherUserTask.id },
    });
...
describe("testing bulk update/delete by an id array", () => {
  ...
  beforeAll(async () => {
    taskX = await prisma.task.create({
      data: { title: "Bulk id task X", userId: user1.id },
    });
    taskY = await prisma.task.create({
      data: { title: "Bulk id task Y", userId: user1.id },
    });
    otherUserTask = await prisma.task.create({
      data: { title: "Other user's task", userId: user2.id },
    });
  });
